...
var addTrafficMarkerInMap = function (trafficInCart, map) {
	var geo = trafficInCart.get("atLocation");
	var trafficPostion = new google.maps.LatLng(geo.latitude, geo.longitude);
    var marker = createMarker(trafficPostion, "traffic");
    marker.setMap(map);
    var infowindow = new google.maps.InfoWindow({
        content: trafficInCart.get("trafficType")
    });
    infowindow.open(map, marker);
    marker.addListener('click', function () {
        infowindow.open(map, marker);
